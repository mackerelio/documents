...
- action.env: action.commandに渡す環境変数を指定することができます。TOMLの[Table][]もしくは[Inline Table][]で指定します。
- memo: チェック監視に対してメモを設定することができます。ここで指定した文字列は、アラート通知・アラート詳細画面・ホスト詳細ページで確認することができます。
