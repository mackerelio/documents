...
以下のように正規表現を用いて指定することで、指定されたファイルシステムからのメトリックは収集されなくなります。なお、メトリック収集の対象は `/dev/` で始まる名前のファイルシステムのみなので、これに該当しないファイルシステムについては除外する必要はありません。
...
**注意**：ホスト詳細画面の「ホスト情報」にある「ファイルシステム」の情報は非表示になりません。
