...
| オプション            | 省略形 | 説明                                                                                      | 複数指定 | 初期値  |
| --------------------- | ------ | ----------------------------------------------------------------------------------------- | -------- | ------- |
| --file                | -f     | 監視対象ファイルのパスを指定（glob形式での指定可）                                        |          |         |
| --pattern             | -p     | 検出したいパターンを正規表現で指定                                                        | ◯        |         |
| --exclude             | -E     | 検出から除外するパターンを正規表現で指定                                                  | ◯        |         |
...
| --return              | -r     | パターンにマッチしたログ行をアラートで通知する *1                                         |          |         |
...
- *1 mackerel-agentによって1024文字を超えた内容は切り詰め処理されます。
