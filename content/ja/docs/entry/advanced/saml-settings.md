...
| セッション有効時間 | 認証されたセッションの有効期限を1〜720（時間）の間で設定します。無指定の場合、24時間となります。設定した場合でも、IdPが指定した有効時間が優先されます。 | 
