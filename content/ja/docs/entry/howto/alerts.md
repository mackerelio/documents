...
<h4 id="calculate-average-value">平均値監視における平均値の算出方法</h4>

平均値は、最新のメトリックを含む、直近の指定点数ぶんのメトリックの合計値から算出されます。指定した時間（分）ではない点に注意してください。

以下は最新のメトリックのタイムスタンプが 15:10 の場合に、平均値監視がどのタイムスタンプの値をもとに算出されるかを示した例です。
...
また、次の監視タイミングで最新のメトリックのタイムスタンプが 15:11 になった場合は、以下のタイムスタンプの値から平均値が算出されます。
...
特定の通知チャンネルへの通知失敗が一定期間続くと、その通知チャンネルは **サスペンド（一時停止）** 状態になります。サスペンドされた通知チャンネルへの通知は行われません。

サスペンドされた通知チャンネルがある場合、サイドメニューの「チャンネル」や、チャンネル一覧ページに警告が表示されます。

![](https://cdn-ak.f.st-hatena.com/images/fotolife/m/mackerelio/20260120/20260120124020.png)

サスペンドを解除するには、通知チャンネルの横にある⚠️アイコンをクリックします。

![](https://cdn-ak.f.st-hatena.com/images/fotolife/m/mackerelio/20260120/20260120124116.png)

**注意**：通知失敗の原因が解消されていない状態で解除しても、再度サスペンドされます。必ず通知失敗の原因を解消してから解除してください。
