...
本番環境ではCollector経由を推奨しますが、開発環境や小規模な構成では直接送信でも問題ありません。詳細は[APMの計装手法とデータ収集の仕組み](https://mackerel.io/ja/docs/entry/apm/architecture)で説明しています。

## 次のステップ

トレース、スパンの概念やデータフローを理解できたら、次は計装手法と環境構成を学びましょう。
